...
RESULT_DIR = ROOT / "results" / "lvit_busi"
LVIT_CODE = ROOT / "code" / "LViT"
BERT_CACHE = ROOT / ".cache" / "huggingface"
BERT_HUB_CACHE = BERT_CACHE / "hub"
BERT_MODEL = "google-bert/bert-base-uncased"

TEXT_DIR = BUSI_ROOT / "text_annotations"
TEXT_FILES = (
    TEXT_DIR / "train_BUSI.xlsx",
    TEXT_DIR / "test_BUSI.xlsx",
)
PROVENANCE_PATH = TEXT_DIR / "provenance.json"
EXPECTED_ANNOTATION_SHA256 = {
    "train_BUSI.xlsx": "17ca4dda53f92312cc29df221db9ba96cd1d1c9409b26270fafdcb6f8031ab9d",
    "test_BUSI.xlsx": "93b70ff3e02be6d2362acdb24b616344c65106ee8db237cfbf168dd33b9dd324",
}
...
def verify_annotation_provenance():
    ...
    for path in TEXT_FILES:
        actual = sha256(path)
        expected = EXPECTED_ANNOTATION_SHA256[path.name]
        recorded = provenance["files"][path.name]["sha256"]
        if actual != expected or recorded != expected:
            raise ValueError(
                f"Annotation provenance/hash mismatch for {path}: "
                f"expected {expected}, recorded {recorded}, actual {actual}"
            )
        print(f"Verified MTGT annotation SHA-256 {path.name}: {actual}")
    return provenance
...
def read_xlsx_rows(path):
    ...
    if not rows or [value.strip() for value in rows[0]] != ["Image", "Description"]:
        raise ValueError(f"Expected XLSX columns ['Image', 'Description'] in {path}")
    return rows[1:]


def load_descriptions():
    """Combine MTGT workbooks globally; fixed local CSVs alone assign splits."""
    descriptions = {}
    for path in TEXT_FILES:
        seen_in_file = set()
...
def build_file_index():
    ...
    images = {path.name: path for path in (BUSI_ROOT / "images").iterdir()
              if path.is_file() and path.suffix.lower() in extensions}
    masks = {path.name: path for path in (BUSI_ROOT / "labels").iterdir()
             if path.is_file() and path.suffix.lower() in extensions}
    return images, masks
...
class BUSILViTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with Image.open(self.mask_index[filename]) as source:
            # BUSI palette masks encode foreground as 1 rather than 255.
            mask = Image.fromarray(
                (np.asarray(source) > 0).astype(np.uint8) * 255,
                mode="L",
            )
        image = TF.resize(
            image,
...
def main(args):
    ...
    provenance = verify_annotation_provenance()
    ...
    expected_samples = {"train": 452, "val": 97, "test": 98}
    for split, values in coverage.items():
        if values != {
            "samples": expected_samples[split],
            "with_text": expected_samples[split],
            "missing": 0,
        }:
            raise ValueError(f"Unexpected fixed-split text coverage for {split}: {values}")
        print(f"Text coverage {split}: {values['with_text']}/{values['samples']}")
    fixed_filenames = set().union(*(set(names) for names in split_names.values()))
    ignored = sorted(set(descriptions) - fixed_filenames)
    if len(ignored) != 133 or not all(name.startswith("normal (") for name in ignored):
        raise ValueError(f"Expected exactly 133 ignored normal records, found {len(ignored)}")
    print("Ignored MTGT normal-image records absent from cleaned BUSI:", len(ignored))
    descriptions = {name: descriptions[name] for name in fixed_filenames}
    ...
    best_path = RESULT_DIR / (
        "best_lvit_busi_smoke.pth" if args.smoke_test else "best_lvit_busi.pth"
    )
    best_epoch = 0
    ...
    if args.smoke_test:
        smoke_metrics = {
            ...
            "text_annotation_provenance": provenance,
        }
