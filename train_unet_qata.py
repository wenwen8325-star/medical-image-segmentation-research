...
def build_qata_index():
    """
    Scan QaTa-COV19 once and build:

    image_index:
        covid_123.png -> full image path

    mask_index:
        covid_123.png -> full mask path

    QaTa masks are named:
        mask_<original_filename>
    """

    image_index = {}
    mask_index = {}

    for root, _, files in os.walk(QATA_ROOT):
        ...
        for filename in files:
            ...
            if filename.startswith("mask_"):
                original_name = filename[len("mask_"):]
                mask_index[original_name] = full_path

            elif "Ground-truths" not in root:
                image_index[filename] = full_path

    return image_index, mask_index


print("Scanning QaTa dataset...")

IMAGE_INDEX, MASK_INDEX = build_qata_index()
...
def main(args):
    ...
    train_dataset = QaTaDataset(
        train_csv,
        image_size=args.image_size,
        augment=True
    )

    val_dataset = QaTaDataset(
        val_csv,
        image_size=args.image_size,
        augment=False
    )

    test_dataset = QaTaDataset(
        test_csv,
        image_size=args.image_size,
        augment=False
    )
