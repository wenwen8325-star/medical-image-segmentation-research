...
QATA_ROOT = ROOT / "datasets" / "QaTa-COV19-v2"
SPLIT_DIR = QATA_ROOT / "splits"
RESULT_DIR = ROOT / "results" / "lvit_qata"
...
TRAIN_TEXT_CSV = Path(
    "/data/students/student2/Full VQA Dataset/Real Dataset/"
    "Train (csv, cleaned, json)/Train_text_for_Covid19.csv"
)
TEST_TEXT_CSV = Path(
    "/data/students/student2/Full VQA Dataset/Real Dataset/"
    "Test (csv,cleaned, json)/Test_text_for_Covid19).csv"
)
...
def load_descriptions():
    ...
    for path in (TRAIN_TEXT_CSV, TEST_TEXT_CSV):
        frame = pd.read_csv(path, usecols=["Image", "Description"])
        frame["Image"] = frame["Image"].astype(str)
        frame["Description"] = frame["Description"].astype(str)

        if frame["Image"].duplicated().any():
            duplicates = frame.loc[frame["Image"].duplicated(), "Image"].tolist()
            raise ValueError(f"Duplicate text filenames in {path}: {duplicates[:5]}")

        for filename, description in frame.itertuples(index=False, name=None):
...
def build_file_index():
    ...
    for path in QATA_ROOT.rglob("*"):
        if not path.is_file() or path.suffix.lower() not in {".png", ".jpg", ".jpeg"}:
            continue
        if path.name.startswith("mask_"):
            masks[path.name[len("mask_"):]] = path
        elif "Ground-truths" not in path.parts:
...
def build_text_embeddings(descriptions, device, batch_size):
    ...
    source_hashes = {
        str(TRAIN_TEXT_CSV): sha256(TRAIN_TEXT_CSV),
        str(TEST_TEXT_CSV): sha256(TEST_TEXT_CSV),
    }
...
def main(args):
    ...
    best_path = RESULT_DIR / "best_lvit_qata_full50.pth"
    best_epoch = 0
    ...
    metrics_path = RESULT_DIR / "final_metrics_full50.json"
